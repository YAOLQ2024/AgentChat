...
**AgentChat 仍然通过 `config.yaml` 组织配置，但敏感值建议放在 `docker/.env` 中，并在 `docker_config.yaml` 里通过 `${VAR}` 引用。**

在启动 Docker 容器之前，请先检查这两个文件：
...
4. **API 密钥**：优先填写到 `docker/.env`，`docker_config.yaml` 中通过占位符引用
