...
import { createWorkspaceSessionAPI, getWorkspacePluginsAPI, workspaceSimpleChatStreamAPI, type WorkSpaceSimpleTask } from '../../../apis/workspace'
...
const isCurrentSessionPersisted = ref(false)
const isSyncingLocalSessionToRoute = ref(false)
const isLoadingSessionHistory = ref(false)
let latestSessionHistoryRequestId = 0
...
const hasVisibleConversation = computed(
  () => messages.value.length > 0 || isLoadingSessionHistory.value
)
...
const ensureCurrentSessionExists = async () => {
  if (isCurrentSessionPersisted.value) {
    return true
  }
  ...
  if (route.query.session_id !== currentSessionId.value) {
    isSyncingLocalSessionToRoute.value = true
    try {
      ...
      isSyncingLocalSessionToRoute.value = false
    }
  }
...
const handleSend = async () => {
  ...
  if (isLoadingSessionHistory.value) {
    ElMessage.warning('正在切换会话，请稍候')
    return
  }
...
const loadSessionHistory = async (sessionId: string) => {
  const requestId = ++latestSessionHistoryRequestId
  ...
  try {
    ...
    if (response.data.status_code === 200) {
      ...
      const nextMessages = session && session.contexts && Array.isArray(session.contexts)
        ? session.contexts.map((ctx: any) => [
          { role: 'user' as const, content: ctx.query || '' },
          { role: 'assistant' as const, content: ctx.answer || '' }
        ]).flat().filter((msg: any) => msg.content) // 过滤掉空内容
        : []

      if (requestId !== latestSessionHistoryRequestId) {
        return
      }
...
watch(
  ...
  async (newSessionId, oldSessionId) => {
    if (newSessionId && newSessionId !== oldSessionId) {
      if (
        isSyncingLocalSessionToRoute.value &&
        newSessionId === currentSessionId.value
      ) {
        console.log('忽略本地新建会话的路由同步:', newSessionId)
        isCurrentSessionPersisted.value = true
        return
      }
...
<template>
  <div class="chat-page" :class="{ 'chat-active': hasVisibleConversation }">
    <div class="chat-container">
      <!-- 欢迎区域（有对话时隐藏） -->
      <div v-if="!hasVisibleConversation" class="welcome-section">
      ...
      <!-- 模式选择（有对话时隐藏） -->
      <div v-if="!hasVisibleConversation" class="mode-selector">
        <button
          v-for="mode in modes"
          :key="mode.id"
      ...
      <!-- 对话历史（有对话时显示在上方） -->
      <div v-if="hasVisibleConversation" class="chat-conversation" ref="chatConversationRef">
        <div v-if="isLoadingSessionHistory" class="session-loading-mask">
      ...
      <!-- 输入区域（固定在底部） -->
      <div class="input-section" :class="{ 'input-fixed': hasVisibleConversation }">
        <div class="input-wrapper" :class="{ 'lingseek-glow': selectedMode === 'lingseek' }">
          ...
          <div class="input-footer">
            ...
            <div class="footer-right">
              ...
              <!-- 发送按钮 -->
              <button class="send-btn" :class="{ 'btn-disabled': isGenerating || isLoadingSessionHistory }" :disabled="isGenerating || isLoadingSessionHistory" @click="handleSend">
...
.chat-conversation {
  ...
  .session-loading-mask {
    ...
    .session-loading-spinner {
      ...
      animation: session-switch-spin 0.9s linear infinite;
    }
  }
