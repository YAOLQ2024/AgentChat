...
sync_mysql_endpoint = app_settings.mysql.get('endpoint')
async_mysql_endpoint = app_settings.mysql.get('async_endpoint') or ""
# aiomysql 和 SQLAlchemy 的 pre-ping 组合会触发 ping 签名不兼容，
# 导致每次异步查询在真正执行前就报错。
async_pool_pre_ping = not async_mysql_endpoint.startswith("mysql+aiomysql://")

engine = create_engine(
    url=sync_mysql_endpoint,
