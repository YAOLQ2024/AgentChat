...
from agentchat.schemas.workspace import WorkSpaceSimpleTask, WorkSpaceSessionCreateRequest
...
async def create_workspace_session(*,
                                   req: WorkSpaceSessionCreateRequest,
                                   login_user: UserPayload = Depends(get_login_user)):
    try:
        if req.session_id:
            result = await WorkSpaceSessionService.get_workspace_session_from_id(req.session_id, login_user.user_id)
            if result:
                return resp_200(data=result)

        result = await WorkSpaceSessionService.create_workspace_session(
